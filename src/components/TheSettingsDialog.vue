<template>
  ...
  >
    ...
    >
      ...
      <v-card-text
        ref="settingsBody"
        class="settings-body pa-0"
      >
        <v-alert
          v-if="qualityRecommendation"
          type="info"
          class="ma-3"
        >
          Repeated buffering detected. A lower quality may help a slow connection, but may
          require Plex transcoding. Server or browser problems may need a different fix.
          <v-btn
            class="mt-2"
            :disabled="isChangingSource"
            @click="ACCEPT_QUALITY_RECOMMENDATION"
          >
            Try {{ qualityRecommendation.label }}
          </v-btn>
        </v-alert>
        <v-list>
          ...
          <v-list-item @click="SET_SHOW_BUFFERING_NOTIFICATIONS(!GET_SHOW_BUFFERING_NOTIFICATIONS)">
            <v-list-item-title>Show buffering popups</v-list-item-title>
            <v-list-item-subtitle>
              Show a popup when another viewer buffers. Diagnostic logging stays enabled.
            </v-list-item-subtitle>
            <template #append>
              <v-switch
                hide-details
                color="primary"
                aria-label="Show buffering popups"
                :model-value="GET_SHOW_BUFFERING_NOTIFICATIONS"
                @update:model-value="SET_SHOW_BUFFERING_NOTIFICATIONS"
                @click.stop
              />
            </template>
          </v-list-item>
            ...
            <v-expansion-panel-text>
              <v-list density="compact">
                <v-list-item
                  ...
                  title="Watch-party connection"
                  subtitle="Use another SyncLounge service. Your Plex libraries are selected in Browse."
                ...
                <v-list-item>
                  ...
                  <v-list-item-subtitle>
                    {{ roomSyncPresetActive ? 'Controlled by the host’s room synchronization preset'
...
export default {
  ...
  computed: {
    ...mapGetters('settings', [
      'GET_AUTOPLAY',
      'GET_SHOW_BUFFERING_NOTIFICATIONS',
      'GET_SLPLAYERFORCETRANSCODE',
    ...
    roomSyncPresetActive() {
      const room = this.$store.state.synclounge;
      return room.isInRoom && room.syncPreset && room.syncPreset !== 'personal';
    },
  ...
  methods: {
    backToSettings() {
      this.advancedOpen = undefined;
      this.$nextTick(() => {
        const body = this.$refs.settingsBody?.$el;
        if (body) body.scrollTop = 0;
      });
    },
    ...mapActions('slplayer', ['ACCEPT_QUALITY_RECOMMENDATION']),
    ...
    ...mapMutations('settings', [
      'SET_AUTOPLAY',
      'SET_SHOW_BUFFERING_NOTIFICATIONS',
